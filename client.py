...
####### Calling exchange secret and pub key functions
        ...
        self.s.send(self.username.encode()) # Inform the server about the username connected 
        print(colored('[+] Connected successfully', 'green'))
        print(colored('[+] Exchanging keys', 'green'))
        ...
        print(colored('[+] Chat app setup completed', 'green'))
        print(colored('[+] Now you can chat', 'green'))
    ...
    def input_handler(self):
        while True:
            ...
            if message == "EXIT":           # EXIT will close down the client
                ...
                key = secret_key
                cipher = AES.new(key, AES.MODE_CFB)                             # Initialize AES object for encryption, parameters: [key], [counter feedback mode]
                ...
                iv = b64encode(cipher.iv).decode('utf-8')                       # Generate the initialization vector, b64encode it, than utf-8 representation to send
                message = b64encode(encrypted_message).decode('utf-8')          # Same process to the encrypted message, to overcome special chars
...
###### Receiving the secret key for symmetric encryption
    def handle_secret(self):
            secret_key = self.s.recv(1024)          # The secret key sent from the server used for symmetric encryption and decryption
            private_key = RSA.importKey(open('client_private_key.pem', 'r').read())    # Import the client private key to decrypt the secret key
...
###### The secret is generated by the server and used for symmetric encryption
    def exchange_public_keys(self):
        try:
            ...
            print(colored('[+] Exchange completed!', 'blue'))
